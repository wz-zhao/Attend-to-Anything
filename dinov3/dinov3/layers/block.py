...
from .moe import MoEAdapterLayer 
...
class SelfAttentionBlock(nn.Module):
    def __init__(
        ...
        if self.use_moe:
            ...
            self.moe = MoEAdapterLayer(dim, text_dim=text_dim, num_experts=8)
        else:
            self.moe = None
    ...
    def _forward(self, x: Tensor, rope=None, text_emb=None, H=None, W=None) -> Tensor:
        b, _, _ = x.shape
        ...
        if self.use_moe and self.moe is not None and text_emb is not None:
            ...
            x_ffn = self.moe(x_ffn, H, W, text_emb)

        return x_ffn

                                         
    def _forward_list(self, x_list: List[Tensor], rope_list=None, text_emb=None, shape_info_list=None) -> List[Tensor]:
        ...
        if self.use_moe and self.moe is not None and text_emb is not None:
            ...
            if shape_info_list is None:
                shape_info_list = [None] * len(x_ffn)
            
            x_moe_out = []
                                                        
            for x_item, shape_hw in zip(x_ffn, shape_info_list):
                if shape_hw is None:
                    N = x_item.shape[1]
                    h = w = int((N - 1)**0.5)
                else:
                ...
                out = self.moe(x_item, h, w, text_emb)
                x_moe_out.append(out)
    ...
    def forward(self, x_or_x_list, rope_or_rope_list=None, text_emb=None, shape_info_list=None) -> List[Tensor]:
        if isinstance(x_or_x_list, Tensor):
                   
            shape_info = shape_info_list[0] if shape_info_list else None
            H, W = shape_info if shape_info else (None, None)
                                                                         
                                       
            return self._forward_list([x_or_x_list], rope_list=[rope_or_rope_list], text_emb=text_emb, shape_info_list=[shape_info])[0]
            ...
            return self._forward_list(x_or_x_list, rope_list=rope_or_rope_list, text_emb=text_emb, shape_info_list=shape_info_list)
        else:
