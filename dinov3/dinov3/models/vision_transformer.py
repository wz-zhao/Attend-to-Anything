...
class DinoVisionTransformer(nn.Module):
    ...
    text_emb: Optional[Tensor] = None,) -> List[Dict[str, Tensor]]:
        x = []
        rope = []
        ...
        for _, blk in enumerate(self.blocks):
            ...
            x = blk(
            x,
            rope_list=rope_sincos,
            text_emb=text_emb,
            shape_info_list=rope,)
    ...
    def forward_features(self, x: Tensor | List[Tensor], masks: Optional[Tensor] = None, text_emb: Optional[Tensor] = None) -> List[Dict[str, Tensor]]:
        if isinstance(x, torch.Tensor):
            return self.forward_features_list([x], [masks], text_emb=text_emb)[0]
        else:
            return self.forward_features_list(x, masks, text_emb=text_emb)

    


                     
    def _get_intermediate_layers_not_chunked(self, x: Tensor, n: int = 1, text_emb=None) -> List[Tensor]:
        ...
        for i, blk in enumerate(self.blocks):
            ...
            x = blk(x, rope_sincos, text_emb=text_emb, shape_info_list=[(H, W)]) 
    ...
    def get_intermediate_layers(
        ...
        *,
        ...
        outputs = self._get_intermediate_layers_not_chunked(x, n, text_emb=text_emb)
        if norm:
            outputs_normed = []
            for out in outputs:
    ...
    def forward(self, *args, is_training: bool = False, text_emb: Optional[Tensor] = None, **kwargs) -> List[Dict[str, Tensor]] | Tensor:
                                
        ret = self.forward_features(*args, text_emb=text_emb, **kwargs)
        if is_training:
            return ret
        else:
